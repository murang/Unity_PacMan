...
public class PlayerController : MonoBehaviour {
	...
	private Stage m_map;
	private GameControl m_gameCtrl;
	...
	void Start () {
		...
		m_gameCtrl = FindObjectOfType(typeof(GameControl)) as GameControl;
		m_weapon = GetComponent<Weapon>();
		m_gameCtrl.AddObjectToList(gameObject);
		m_map = FindObjectOfType(typeof(Stage)) as Stage;
	}

	void OnStageStart()
	{
		ChangeState("State_Normal",State_NormalInit);
		transform.position = m_map.GetSpawnPoint(Stage.SPAWN_POINT_TYPE.PLAYER);
